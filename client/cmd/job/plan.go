...
func (p *planCommand) inject(cmd *cobra.Command) {
	...
	cmd.Flags().StringVarP(&p.output, "output", "o", "./plan.csv", "File Output Path")
...
func (p *planCommand) getAllDirectories(diffs []*git.Diff) []string {
	...
	for i := range diffs {
		directories = p.appendPath(diffs[i].OldPath, pathExists, directories)
		directories = p.appendPath(diffs[i].NewPath, pathExists, directories)
	}

	return directories
}

func (*planCommand) appendPath(path string, pathExists map[string]bool, filePaths []string) []string {
	index := strings.Index(path, "/assets")
	if !strings.HasSuffix(path, "/"+jobFileName) && index < 1 {
		return filePaths
	}

	path = strings.TrimSuffix(path, "/"+jobFileName)
	if index > 0 {
		path = path[:index]
	}
...
func (p *planCommand) saveFile(plans plan.Plans) error {
	...
	if fileInfo.Size() > 0 {
		var existingPlans plan.Plans
		if err = gocsv.UnmarshalFile(file, &existingPlans); err != nil {
			p.logger.Error("[plan] failed unmarshal existing file, replacing old file: %s", fileInfo.Name())
		} else {
			plans = append(existingPlans, plans...)
		}
	}
	...
	if p.verbose {
		p.logger.Info("[plan] file plan created at: %s", file.Name())
	}
	return nil
}
