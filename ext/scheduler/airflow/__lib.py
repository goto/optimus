...
STARTUP_TIMEOUT_IN_SECS = int(
    Variable.get("startup_timeout_in_secs", default_var=2 * 60)
)
OPTIMUS_REQUEST_TIMEOUT_IN_SECS = int(
    Variable.get("optimus_request_timeout_in_secs", default_var=5 * 60)
)
...
class SuperKubernetesPodOperator(KubernetesPodOperator):
    def __init__(self, *args, **kwargs):
        kwargs["node_selectors"] = {"optimus-jobs": "enabled"}
        kwargs["tolerations"] = [
            k8s.V1Toleration(key="optimus-jobs", operator="Equal", value="enabled")
        ]
...
class OptimusAPIClient:
    ...
    def get_job_run(
        self, optimus_project: str, optimus_job: str, startDate: str, endDate: str
    ) -> dict:
    ...
    def get_job_run_input(
        ...
        response = requests.post(
            url="{}/api/v1beta1/project/{}/job/{}/run_input".format(
                self.host, project_name, job_name
            ),
            json={
    ...
    def notify_event(self, project, namespace, job, event) -> dict:
        ...
        response = requests.post(
            url, data=json.dumps(request_data), timeout=OPTIMUS_REQUEST_TIMEOUT_IN_SECS
        )
        self._raise_error_if_request_failed(response)
        return response.json()
    ...
    def _raise_error_if_request_failed(self, response):
        if response.status_code != 200:
            log.error(
                "Request to optimus returned non-200 status code. Server response:\n"
            )
            log.error(response.json())
            raise AssertionError(
                "request to optimus returned non-200 status code. url: " + response.url
            )
...
class JobSpecTaskWindow:
    def __init__(
        ...
        offset: str,
        truncate_to: str,
        optimus_client: OptimusAPIClient,
    ):
    ...
    def get_schedule_window(
        self, scheduled_at: str, upstream_schedule: str
    ) -> (str, str):
        api_response = self._fetch_task_window(scheduled_at)
    ...
    def _fetch_task_window(self, scheduled_at: str) -> dict:
        return self._optimus_client.get_task_window(
            scheduled_at, self.version, self.size, self.offset, self.truncate_to
        )
...
class SuperExternalTaskSensor(BaseSensorOperator):
    ...
    def poke(self, context):
        job_cron_iter = croniter(
            context.get("dag").schedule_interval, context.get("execution_date")
        )
        ...
        try:
            ...
            context[
                SCHEDULER_ERR_MSG
            ] = "error while fetching upstream schedule :: {}".format(e)
            return False
        ...
        # get schedule window
        task_window = JobSpecTaskWindow(
            self.window_version, self.window_size, 0, "", self._optimus_client
        )
        (
            schedule_time_window_start,
            schedule_time_window_end,
        ) = task_window.get_schedule_window(
            last_upstream_schedule_time.strftime(TIMESTAMP_FORMAT), upstream_schedule
        )

        self.log.info(
            "waiting for upstream runs between: {} - {} schedule times of airflow dag run".format(
                schedule_time_window_start, schedule_time_window_end
            )
        )

        # a = 0/0
        if not self._are_all_job_runs_successful(
            schedule_time_window_start, schedule_time_window_end
        ):
            self.log.warning(
                "unable to find enough successful executions for upstream '{}' in "
                "'{}' dated between {} and {}(inclusive), rescheduling sensor".format(
                    self.optimus_job,
                    self.optimus_project,
                    schedule_time_window_start,
                    schedule_time_window_end,
                )
            )
    ...
    def get_last_upstream_times(
        self, schedule_time_of_current_job, upstream_schedule_interval
    ):
        second_ahead_of_schedule_time = schedule_time_of_current_job + timedelta(
            seconds=1
        )
    ...
    def get_schedule_interval(self, schedule_time):
        ...
        job_metadata = self._upstream_optimus_client.get_job_metadata(
            schedule_time_str,
            self.optimus_namespace,
            self.optimus_project,
            self.optimus_job,
        )
        upstream_schedule = lookup_non_standard_cron_expression(
            job_metadata["spec"]["interval"]
        )
    ...
    def _are_all_job_runs_successful(
        self, schedule_time_window_start, schedule_time_window_end
    ) -> bool:
        try:
...
def optimus_notify(context, event_meta):
    ...
    job_cron_iter = croniter(
        context.get("dag").schedule_interval, current_execution_date
    )
    ...
    def _xcom_value_has_error(_xcom) -> bool:
        return (
            _xcom.key == XCOM_RETURN_KEY
            and isinstance(_xcom.value, dict)
            and "error" in _xcom.value
            and _xcom.value["error"] is not None
        )
    ...
    # post event
    log.info(event)
    resp = optimus_client.notify_event(
        params["project_name"], params["namespace"], params["job_name"], event
    )
...
def job_success_event(context):
    try:
        meta = {"event_type": "TYPE_JOB_SUCCESS", "status": "success"}
...
def job_failure_event(context):
    try:
        meta = {"event_type": "TYPE_FAILURE", "status": "failed"}
        result_for_monitoring = get_result_for_monitoring_from_xcom(context)
        if result_for_monitoring is not None:
            meta["monitoring"] = result_for_monitoring
    ...
    try:
        ...
        if run_type == "SENSOR":
            if not shouldSendSensorStartEvent(context):
                return
        meta = {"event_type": "TYPE_{}_START".format(run_type), "status": "running"}
        optimus_notify(context, meta)
    except Exception as e:
        print(e)
...
def operator_success_event(context):
    try:
        run_type = get_run_type(context)
        meta = {"event_type": "TYPE_{}_SUCCESS".format(run_type), "status": "success"}
        optimus_notify(context, meta)
    except Exception as e:
        print(e)
...
def operator_retry_event(context):
    try:
        run_type = get_run_type(context)
        meta = {"event_type": "TYPE_{}_RETRY".format(run_type), "status": "retried"}
        optimus_notify(context, meta)
    except Exception as e:
        print(e)
...
def operator_failure_event(context):
    try:
        run_type = get_run_type(context)
        meta = {"event_type": "TYPE_{}_FAIL".format(run_type), "status": "failed"}
            ...
            sla_list.append(
                {
                    "task_id": sla.task_id,
                    "dag_id": sla.dag_id,
                    "scheduled_at": dag.following_schedule(sla.execution_date).strftime(
                        TIMESTAMP_FORMAT
                    ),
...
def shouldSendSensorStartEvent(ctx):
    try:
        ...
        if len(task_reschedules) == 0:
            log.info(
                f"sending NEW sensor start event for attempt number-> {ti.try_number}"
            )
            return True
...
def alert_failed_to_slack(context):
    ...
    try:
        ...
        return "{connection} connection variable not defined, unable to send alerts".format(
            connection=SLACK_CONN_ID
        )
    ...
    message_footer = "\n".join(
        [
            ...
            ":hourglass: Duration: {} sec".format(
                context.get("task_instance").duration
            ),
    ...
    blocks = [
        {"type": "section", "text": {"type": "mrkdwn", "text": "Task failed :fire:"}},
        {"type": "section", "text": {"type": "mrkdwn", "text": message_body}},
        {
            "type": "actions",
            "elements": [
                {
        ...
        {"type": "divider"},
        {
            "type": "context",
            "elements": [
                {"type": "mrkdwn", "text": message_footer},
            ],
        },
    ]
