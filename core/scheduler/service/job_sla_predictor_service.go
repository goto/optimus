...
func (s *JobSLAPredictorService) PredictJobSLAs(ctx context.Context, jobs []*scheduler.JobSchedule, targetedSLA time.Time) (map[*scheduler.JobLineageSummary][]*scheduler.JobLineageSummary, map[*scheduler.JobLineageSummary]*JobSLAState, error) {
	...
	jobSLAStates := make(map[*scheduler.JobLineageSummary]*JobSLAState)
	...
	jobSLABreachCauses, err := s.identifyPotentialBreachJobs(jobsWithLineage, jobSLAStates, time.Now().UTC())
	if err != nil {
		return nil, nil, err
	}

	return jobSLABreachCauses, jobSLAStates, nil
}
...
func (s *JobSLAPredictorService) calculateEstimatedDuration(ctx context.Context, jobNames []scheduler.JobName, jobsWithLineage []*scheduler.JobLineageSummary, jobSLAStates map[*scheduler.JobLineageSummary]*JobSLAState, targetedSLA time.Time) (map[*scheduler.JobLineageSummary]*JobSLAState, error) {
	...
	for len(queue) > 0 {
		...
		if duration, ok := jobDurations[job.JobName]; ok {
			...
			jobSLAStates[job].EstimatedDuration = &estimatedDuration
		}
		queue = append(queue, job.Upstreams...)
	}
...
func (s *JobSLAPredictorService) calculateInferredSLAs(jobsWithLineage []*scheduler.JobLineageSummary, jobSLAStates map[*scheduler.JobLineageSummary]*JobSLAState, targetedSLA time.Time) (map[*scheduler.JobLineageSummary]*JobSLAState, error) {
	for _, jobTarget := range jobsWithLineage {
		// inferred SLA for leaf node = targetedSLA S(j)
		jobSLAStates[jobTarget].InferredSLAByJobName[jobTarget.JobName] = &targetedSLA
		...
		for len(stack) > 0 {
			...
			targetedInferredSLA := jobSLAStates[job].InferredSLAByJobName[jobTarget.JobName]
			if jobSLAStates[job].EstimatedDuration == nil || targetedInferredSLA == nil {
				continue
			}
			inferredSLA := targetedInferredSLA.Add(-*jobSLAStates[job].EstimatedDuration)
			for _, upstreamJob := range job.Upstreams {
				jobSLAStates[upstreamJob].InferredSLAByJobName[jobTarget.JobName] = &inferredSLA
...
func (s *JobSLAPredictorService) identifyPotentialBreachJobs(jobsWithLineage []*scheduler.JobLineageSummary, jobSLAStates map[*scheduler.JobLineageSummary]*JobSLAState, referenceTime time.Time) (map[*scheduler.JobLineageSummary][]*scheduler.JobLineageSummary, error) {
	...
	for _, jobTarget := range jobsWithLineage {
		...
		for len(stack) > 0 {
			...
			if jobSLAStates[job].InferredSLAByJobName[jobTarget.JobName] == nil || jobSLAStates[job].EstimatedDuration == nil {
				continue
			}
			for _, upstreamJob := range job.Upstreams {
			...
			inferredSLA := *jobSLAStates[job].InferredSLAByJobName[jobTarget.JobName]
			...
			if referenceTime.After(inferredSLA.Add(-*jobSLAStates[job].EstimatedDuration)) && job.JobRuns[jobTarget.JobName.String()].TaskStartTime == nil {
				// found a job that might breach its SLA, return the jobTarget
				potentialBreachPaths[jobTarget] = append(potentialBreachPaths[jobTarget], paths)
			}
		}
