...
// defaultJanitorInterval caps the sweep period for a very long TTL.
...
// coalesce into one load (avoiding a stampede on a cold key), expired entries are
// swept periodically, and failed loads are never cached.
...
// New creates a Cache with the given TTL. A ttl <= 0 disables caching: every GetOrLoad
// call invokes load directly and nothing is stored.
...
// GetOrLoad returns the cached value for key if present and unexpired, otherwise calls
// load exactly once (even under concurrent callers) and caches a successful result.
...
// Len reports the number of unexpired entries; for observability, not hot-path use.
...
// Close stops the background janitor goroutine; safe to call multiple times or never.
