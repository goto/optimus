...
func TestReplayService(t *testing.T) {
	...
	replayConfig := scheduler.NewReplayConfig(startTime, endTime, parallel, replayJobConfig, description, "", "approval_id", "user_id")
	replayID := uuid.New()
	job := scheduler.Job{
		Name:   jobName,
		Tenant: tnnt,
	...
	t.Run("CreateReplay", func(t *testing.T) {
		...
		t.Run("should use namespace config EXECUTION_PROJECT if not provided in replay config", func(t *testing.T) {
			...
			replayConfigEmptyJobConfig := scheduler.NewReplayConfig(startTime, endTime, parallel, map[string]string{}, description, "", "approval_id", "user_id")
			replayConfigWithNamespaceConfig := scheduler.NewReplayConfig(startTime, endTime, parallel, map[string]string{"EXECUTION_PROJECT": "example_project_from_namespace"}, description, "", "approval_id", "user_id")
...
func (_m *ReplayRepository) GetReplayByApprovalID(ctx context.Context, approvalID string) (*scheduler.ReplayWithRun, error) {
	args := _m.Called(ctx, approvalID)
	return args.Get(0).(*scheduler.ReplayWithRun), args.Error(1)
}
...
func (_m *ReplayValidator) Validate(ctx context.Context, replayRequest service.ReplayRequest, jobCron *cron.ScheduleSpec) error {
	ret := _m.Called(ctx, replayRequest, jobCron)
	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, service.ReplayRequest, *cron.ScheduleSpec) error); ok {
...
// ValidateBackfill provides a mock function with given fields: ctx, replayRequest
func (_m *ReplayValidator) ValidateBackfill(ctx context.Context, replayRequest service.ReplayRequest, jobCron *cron.ScheduleSpec) error {
	args := _m.Called(ctx, replayRequest, jobCron)
	return args.Error(0)
}
