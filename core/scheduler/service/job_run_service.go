...
func (s *JobRunService) registerSLAs(ctx context.Context, eventCtx *scheduler.EventContext, slaAlertConfigs []*scheduler.SLAAlertConfig) error {
	...
	for _, slaAlertConfig := range slaAlertConfigs {
		...
		if err != nil {
			s.l.Error("[Register SLA] unable to get sla duration, skipping sla registration for jobRunID: %s, operatorName: %s, operatorType: %s, sla: %s, err: %s", jobRunID, operatorName, operatorType.String(), slaAlertConfig.Tag(), err.Error())
			return err
		}
		if slaDuration == nil {
			s.l.Warn("[Register SLA] sla duration is nil, skipping sla registration for jobRunID: %s, operatorName: %s, operatorType: %s, sla: %s", jobRunID, operatorName, operatorType.String(), slaAlertConfig.Tag())
			return nil
		}
		...
		if err != nil {
			errMsg := fmt.Sprintf("[Register SLA] error registering sla for operator Run Id: %s, Type: %s, Sla: %s, err: %s", jobRunID, operatorType.String(), slaAlertConfig.Tag(), err.Error())
