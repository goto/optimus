...
func (s StatusRepository) Touch(ctx context.Context, projectName tenant.ProjectName, identifiers []string) error {
	updateQuery := `update sync_status set last_sync_attempt = NOW() where project_name=$1 and identifier  in ('` + strings.Join(identifiers, "', '") + `')`
	_, err := s.db.Exec(ctx, updateQuery, projectName)
	if err != nil {
