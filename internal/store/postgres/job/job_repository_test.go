...
func TestPostgresJobRepository(t *testing.T) {
	...
	t.Run("Add", func(t *testing.T) {
		t.Run("inserts job spec", func(t *testing.T) {
			...
			jobSpecA, err := job.NewSpecBuilder(jobVersion, "sample-job-A", jobOwner, jobSchedule, customConfig, jobTask).
				...
				Build()
			assert.NoError(t, err)
			jobA := job.NewJob(sampleTenant, jobSpecA, resourceURNA, []resource.URN{resourceURN3}, false, job.ENABLED)
		...
		t.Run("skip job and return job error if job already exist", func(t *testing.T) {
			...
			assert.NoError(t, err)
			jobB := job.NewJob(sampleTenant, jobSpecB, resourceURNB, []resource.URN{resourceURN3}, false, job.ENABLED)
		...
		t.Run("avoid re-inserting job if it is soft deleted in other namespace", func(t *testing.T) {
			...
			jobAToReAdd := job.NewJob(otherTenant, jobSpecA, resourceURNA, []resource.URN{resourceURN3}, false, job.ENABLED)
	...
	t.Run("Update", func(t *testing.T) {
		t.Run("updates job spec", func(t *testing.T) {
			...
			jobAToUpdate := job.NewJob(sampleTenant, jobSpecAToUpdate, resourceURNA, []resource.URN{resourceURN3}, false, job.ENABLED)
			jobBToUpdate := job.NewJob(sampleTenant, jobSpecB, resourceURNB, []resource.URN{resourceURN4}, false, job.ENABLED)
		...
		t.Run("should not update job if it has been soft deleted", func(t *testing.T) {
			...
			otherTenant, err := tenant.NewTenant(proj.Name().String(), otherNamespace.Name().String())
			assert.NoError(t, err)
			jobToUpdate := job.NewJob(otherTenant, jobSpecA, resource.ZeroURN(), nil, false, job.ENABLED)
		...
		t.Run("should not update job if it is owned by different namespace", func(t *testing.T) {
			...
			otherTenant, err := tenant.NewTenant(proj.Name().String(), otherNamespace.Name().String())
			assert.NoError(t, err)
			jobAToUpdate := job.NewJob(otherTenant, jobSpecA, resourceURNA, []resource.URN{resourceURN3}, false, job.ENABLED)
	...
	t.Run("ResolveUpstreams", func(t *testing.T) {
		t.Run("returns job with inferred upstreams", func(t *testing.T) {
			...
			jobSpecA, err := job.NewSpecBuilder(jobVersion, "sample-job-A", jobOwner, jobSchedule, customConfig, jobTask).WithDescription(jobDescription).Build()
			assert.NoError(t, err)
			jobA := job.NewJob(sampleTenant, jobSpecA, resourceURNA, []resource.URN{resourceURNB}, false, job.ENABLED)
		...
		t.Run("returns job with static upstreams", func(t *testing.T) {
			...
			jobSpecA, _ := job.NewSpecBuilder(jobVersion, "sample-job-A", jobOwner, jobSchedule, customConfig, jobTask).
				...
				Build()
			jobA := job.NewJob(sampleTenant, jobSpecA, resourceURNA, nil, false, job.ENABLED)
		...
		t.Run("returns job with static and inferred upstreams", func(t *testing.T) {
			...
			assert.NoError(t, err)
			jobC := job.NewJob(sampleTenant, jobSpecC, resourceURNC, nil, false, job.ENABLED)
		...
		t.Run("returns job with external project static and inferred upstreams", func(t *testing.T) {
			...
			jobSpecA, _ := job.NewSpecBuilder(jobVersion, "sample-job-A", jobOwner, jobSchedule, customConfig, jobTask).
				...
				WithSpecUpstream(jobAUpstream).
				Build()
			jobA := job.NewJob(sampleTenant, jobSpecA, resourceURNA, []resource.URN{resourceURNC, resourceURNE}, false, job.ENABLED)
			...
			jobD := job.NewJob(otherTenant, jobSpecD, resourceURND, nil, false, job.ENABLED)
			...
			jobE := job.NewJob(otherTenant, jobSpecE, resourceURNE, nil, false, job.ENABLED)
	...
	t.Run("ReplaceUpstreams", func(t *testing.T) {
		...
		t.Run("inserts job upstreams with exact name across projects exists", func(t *testing.T) {
			...
			otherProjectJobA := job.NewJob(otherTenant, jobSpecA, resourceURNA, []resource.URN{resourceURNC}, false, job.ENABLED)
			otherProjectJobB := job.NewJob(otherTenant, jobSpecB, resourceURNB, nil, false, job.ENABLED)
	...
	t.Run("Delete", func(t *testing.T) {
		...
		t.Run("do delete job and delete upstream relationship", func(t *testing.T) {
			...
			assert.NoError(t, err)
			jobX := job.NewJob(sampleTenant, jobSpecX, resourceURNX, []resource.URN{resourceURNA}, false, job.ENABLED)
	...
	t.Run("GetByJobName", func(t *testing.T) {
		t.Run("returns job success", func(t *testing.T) {
			...
			jobSpecA, err := job.NewSpecBuilder(jobVersion, "sample-job-A", jobOwner, jobSchedule, customConfig, jobTask).WithDescription(jobDescription).Build()
			assert.NoError(t, err)
			jobA := job.NewJob(sampleTenant, jobSpecA, resourceURNA, []resource.URN{resourceURNB, resourceURNC}, false, job.ENABLED)
	...
	t.Run("GetAllByProjectName", func(t *testing.T) {
		t.Run("returns no error when get all jobs success", func(t *testing.T) {
			...
			jobSpecB, err := job.NewSpecBuilder(jobVersion, "sample-job-B", jobOwner, jobSchedule, customConfig, jobTask).WithDescription(jobDescription).Build()
			assert.NoError(t, err)
			jobB := job.NewJob(sampleTenant, jobSpecB, resourceURNB, []resource.URN{resourceURNC}, false, job.ENABLED)
	...
	t.Run("GetAllByResourceDestination", func(t *testing.T) {
		t.Run("returns no error when get all jobs success", func(t *testing.T) {
			...
			jobSpecA, err := job.NewSpecBuilder(jobVersion, "sample-job-A", jobOwner, jobSchedule, customConfig, jobTask).WithDescription(jobDescription).Build()
			assert.NoError(t, err)
			jobA := job.NewJob(sampleTenant, jobSpecA, resourceURNY, []resource.URN{resourceURNB, resourceURNC}, false, job.ENABLED)
			jobSpecB, err := job.NewSpecBuilder(jobVersion, "sample-job-B", jobOwner, jobSchedule, customConfig, jobTask).WithDescription(jobDescription).Build()
			assert.NoError(t, err)
			jobB := job.NewJob(sampleTenant, jobSpecB, resourceURNY, []resource.URN{resourceURNC}, false, job.ENABLED)
	...
	t.Run("GetDownstreamBySources", func(t *testing.T) {
		...
		t.Run("returns downstream given resource urns", func(t *testing.T) {
			...
			assert.NoError(t, err)
			jobB := job.NewJob(sampleTenant, jobSpecB, resourceURND, []resource.URN{resourceURNE}, false, job.ENABLED)
			...
			assert.NoError(t, err)
			jobC := job.NewJob(sampleTenant, jobSpecC, resourceURNF, nil, false, job.ENABLED)
