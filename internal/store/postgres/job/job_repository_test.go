...
func TestPostgresJobRepository(t *testing.T) {
	...
	t.Run("Upsert", func(t *testing.T) {
		t.Run("inserts job spec if it is not exist yet", func(t *testing.T) {
			db := dbSetup()
			...
			jobRepo := postgres.NewJobRepository(db)
			addedJobs, updatedJobs, err := jobRepo.Upsert(ctx, jobs)
			assert.NoError(t, err)
			assert.EqualValues(t, jobs, addedJobs)
			assert.Equal(t, 0, len(updatedJobs))

			storedJobs, err := jobRepo.GetAllByProjectName(ctx, proj.Name())
			assert.NoError(t, err)
			assert.EqualValues(t, jobs, storedJobs)
		})
		t.Run("updates job spec if it is already exist", func(t *testing.T) {
			db := dbSetup()
			...
			addedJobs, updatedJobs, err := jobRepo.Upsert(ctx, jobsToUpdate)
			assert.NoError(t, err)
			assert.EqualValues(t, jobsToUpdate, updatedJobs)
			assert.Equal(t, 0, len(addedJobs))
		})
		t.Run("inserts job spec if it not exist yet and update if it is already exists", func(t *testing.T) {
			db := dbSetup()
			...
			jobSpecC, err := job.NewSpecBuilder(jobVersion, "sample-job-C", jobOwner, jobSchedule, customConfig, jobTask).Build()
			assert.NoError(t, err)
			...
			jobAToUpdate := job.NewJob(sampleTenant, jobSpecAToUpdate, resourceURNA, []resource.URN{resourceURN3}, false)
			jobBToUpdate := job.NewJob(sampleTenant, jobSpecB, resourceURNB, []resource.URN{resourceURN4}, false)
			jobsToUpsert := []*job.Job{jobAToUpdate, jobBToUpdate, jobC}

			addedJobs, updatedJobs, err := jobRepo.Upsert(ctx, jobsToUpsert)
			assert.NoError(t, err)
			assert.EqualValues(t, []*job.Job{jobAToUpdate, jobBToUpdate}, updatedJobs)
			assert.EqualValues(t, []*job.Job{jobC}, addedJobs)
		})
		t.Run("should update job if it has been soft deleted and to be recreated in the same namespace", func(t *testing.T) {
			db := dbSetup()
			...
			addedJobs, updatedJobs, err := jobRepo.Upsert(ctx, []*job.Job{jobA})
			assert.NoError(t, err)
			assert.Nil(t, addedJobs)
			assert.EqualValues(t, []*job.Job{jobA}, updatedJobs)
		})
		t.Run("should not update job if it has been soft deleted and to be recreated in a different namespace", func(t *testing.T) {
			db := dbSetup()
			...
			assert.NoError(t, err)
			jobToUpdate := job.NewJob(otherTenant, jobSpecA, resource.ZeroURN(), nil, false)
			addedJobs, updatedJobs, err := jobRepo.Upsert(ctx, []*job.Job{jobToUpdate})
			assert.ErrorContains(t, err, "already exists and soft deleted in namespace test-ns")
			assert.Nil(t, updatedJobs)
			assert.Nil(t, addedJobs)
		})
		t.Run("skip job and return job error if one of the job is invalid", func(t *testing.T) {
			db := dbSetup()
			...
			jobSpecB, err := job.NewSpecBuilder(jobVersion, "sample-job-B", jobOwner, jobSchedule, customConfig, jobTask).Build()
			assert.NoError(t, err)
			jobB := job.NewJob(sampleTenant, jobSpecB, resourceURNB, []resource.URN{resourceURN4}, false)
			...
			assert.NoError(t, err)
			jobToUpdate := job.NewJob(otherTenant, jobSpecA, resource.ZeroURN(), nil, false)
			addedJobs, updatedJobs, err := jobRepo.Upsert(ctx, []*job.Job{jobToUpdate, jobB})
			assert.ErrorContains(t, err, "already exists and soft deleted in namespace test-ns")
			assert.Nil(t, updatedJobs)
			assert.EqualValues(t, []*job.Job{jobB}, addedJobs)
		})
