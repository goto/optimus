...
func TestReplayValidator(t *testing.T) {
	...
	replayConfig := scheduler.NewReplayConfig(replayStartTime, replayEndTime, parallel, replayJobConfig, description, "", "approval_id", "user_id")
	...
	t.Run("Validate", func(t *testing.T) {
		t.Run("should return nil if validation valid", func(t *testing.T) {
			...
			onGoingReplayConfig := scheduler.NewReplayConfig(time.Now(), time.Now(), parallel, replayJobConfig, description, "", "approval_id", "user_id")
		...
		t.Run("should not return error if replay end date equals to job's end_date scheduled date", func(t *testing.T) {
			...
			replayJobAConfig := scheduler.NewReplayConfig(replayJobAStartTime, replayJobAEndTime, parallel, replayJobConfig, description, "", "approval_id", "user_id")
		...
		t.Run("should return error if unable to get job runs", func(t *testing.T) {
			...
			onGoingReplayConfig := scheduler.NewReplayConfig(time.Now(), time.Now(), parallel, map[string]string{}, description, "", "approval_id", "user_id")
			onGoingReplay := []*scheduler.Replay{
				scheduler.NewReplayRequest(jobName, tnnt, onGoingReplayConfig, scheduler.ReplayStateCreated),
			}
