...
func TestReplayService(t *testing.T) {
	...
	replayConfig := scheduler.NewReplayConfig(startTime, endTime, parallel, replayJobConfig, description, "", "approver_id", "user_id")
	replayID := uuid.New()
	job := scheduler.Job{
		Name:   jobName,
		Tenant: tnnt,
	...
	t.Run("CreateReplay", func(t *testing.T) {
		...
		t.Run("should use namespace config EXECUTION_PROJECT if not provided in replay config", func(t *testing.T) {
			...
			replayConfigEmptyJobConfig := scheduler.NewReplayConfig(startTime, endTime, parallel, map[string]string{}, description, "", "approver_id", "user_id")
			replayConfigWithNamespaceConfig := scheduler.NewReplayConfig(startTime, endTime, parallel, map[string]string{"EXECUTION_PROJECT": "example_project_from_namespace"}, description, "", "approver_id", "user_id")
...
func (_m *ReplayRepository) GetReplayByApproverID(ctx context.Context, approverID string) (*scheduler.ReplayWithRun, error) {
